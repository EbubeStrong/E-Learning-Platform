...
import { HelpCircle, Award, Clock3, ListChecks, Play } from "lucide-react";
...
export default function QuizCatalogPage() {
  ...
  const myCerts = useQuery(
    ...
    userId ? { userId, quizType: "certification" } : "skip"
  );

  const loading =
    courses === undefined || quizzes === undefined || myCerts === undefined;
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-mocha-400">
          ...
          certificate at 75%+ (max 3 attempts).
        </p>
      </div>
      ...
      {courses?.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          {(courses ?? []).map((course) => {
            ...
            const certificationAttempts = (myCerts ?? []).filter(
              (attempt) => attempt.courseId === course.courseId
            );
            const passedCert = certificationAttempts.some(
              (attempt) => attempt.percent != null && attempt.percent >= (certificationQuiz?.passThreshold ?? 0)
            );
            const attemptsUsed = certificationAttempts.length;
            ...
            return (
              ...
              >
                <div className="mb-3 flex items-center justify-between">
                  <span className="inline-flex items-center gap-1 rounded-full bg-mocha-300/40 px-2.5 py-0.5 text-xs font-medium text-mocha-500">
                    {course.category}
                  </span>
                      ...
                      footer={
                        passedCert ? (
                          ...
                          <span className="text-xs text-mocha-400">
                            {attemptsUsed}/3 attempts used · pass at{" "}
    ...
    >
      ...
      <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-mocha-400">
        ...
        <span className="inline-flex items-center gap-1 font-semibold text-mocha-500">
          <Play className="h-3.5 w-3.5" /> {meta.points} pts
        </span>
      </div>
